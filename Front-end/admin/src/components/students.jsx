  ...
  return (
    <div className={`${Container}`}>
      ...
      <div className={`${styleTopBarUINoFlex} ${open ? "hidden" : "block"} min-h-96 overflow-scroll p-3`}>
        {loading ? (
          <div className="flex items-center justify-center min-h-32"><LOADER /></div>
        ) : students?.length === 0 ? (
          <div className="flex items-center justify-center min-h-32">No students found</div>
        ) : (
                ...
                return <tr key={id}>
                  ...
                  <td>
                    <p className="min-w-max h-full flex items-center justify-start gap-2 min-h-max text-lg font-normal">
                      <img className='w-10 h-10 bg-center rounded-full' hidden={!item?.image} src={item?.image} alt={item?.image} />
                  ...
                  <td>
                    <span className="min-w-max h-full leading-5 flex items-center justify-between relative">
                      <button onClick={() => {
                      ...
                      <div className="dropdown">
                        <button className="flex items-center justify-center" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                          <img src={menuDots} width={23} className=" p-1" alt="menuDots" />
                        </button>
